...
def tok(data: List[str], tokzr: AutoTokenizer, max_len: int):
    """
    Read data and labels from a file
    ...
    max_len: int
        The maximum lenght of an input utterance
    ...
    for sent in data:
        tok_data.append(tokzr.encode(sent)[:max_len])
...
def to_batch(text: List[List[int]], labels: List[int], batch_size: int, padding_id: int, DEVICE: str):
    ...
    for batch_idx in range(num_batches):
        ...
        new_batch_text_mask = torch.zeros((batch_size, max_len), dtype = torch.long, device=DEVICE)
        ...
        for sent_idx in range(batch_size):
            ...
            for word_idx, word_id in enumerate(text[beg_idx + sent_idx]):
                ...
                new_batch_text_mask[sent_idx][word_idx] = 1
        text_batches.append(new_batch_text)
        text_batches_mask.append(new_batch_text_mask)
        label_batches.append(new_batch_labels)

    return text_batches, text_batches_mask, label_batches
